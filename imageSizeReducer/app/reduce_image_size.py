...
class ImageSizeReducer:
    ...
    def calculate_compress_image_size_in_bytes(self, file_path: str) -> int:
        ...
        :return: int
        """
        reduction_percentage = self.reduction_quality_percentage
        compress_file_size_in_byte = self.target_image_size  # bytes

        if not compress_file_size_in_byte:  # notice target_image_size has high priority
            file_size_in_byte = file_functions.get_file_size_in_bytes(file_path)
            compress_file_size_in_byte = file_size_in_byte * (1 - (reduction_percentage / 100))
